...
namespace Project.Models.Maze
{
    public class MaskedColoredGameGrid : ColoredGameGrid,IMaskable
    {
        ...
        public MaskedColoredGameGrid(int rows, int columns) : base(rows, columns)
        {
        }

        public MaskedColoredGameGrid(GenerationSettingsSO generationSettings) : base(generationSettings)
        {
        }

        public MaskedColoredGameGrid(Mask mask) : base(mask.Rows, mask.Columns)
        {
            Mask = mask;
            PrepareGrid();
            ConfigureCells();
